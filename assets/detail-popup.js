...
(function () {
  ...
  /* ---------- 评论 mock 数据 ---------- */
  var COMMENTS = [
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。非常推荐，超级好玩！', time: '2小时前', likes: 999, authorLiked: true, replyCount: 99 },
    { name: '滴栗滴栗', text: '回复 滴栗滴栗：玩家评论，固定展示两行。', time: '2小时前', likes: 999, isReply: true },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。非常推荐，超级好玩！非常推荐，超级好玩！', time: '2小时前', likes: 188 },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。', time: '3小时前', likes: 56 },
    { name: '滴栗滴栗', text: '回复 滴栗滴栗：玩家评论，固定展示两行。', time: '4小时前', likes: 12, isReply: true },
      ...
      '<div class="cm-item" data-i="', idx, '">',
        ...
        '<div class="cm-body">',
          ...
          '<div class="cm-meta">',
            ...
            '</span>',
          '</div>',
          (c.replyCount ? '<button class="cm-reply99" data-act="open-replies">' + c.replyCount + ' 条回复</button>' : ''),
        '</div>',
      '</div>'
    ].join('');
  }
        ...
        '<div class="detail-body">',
          ...
          '<div class="detail-left">',
            '<div class="dl-card">',
              ...
              '<div class="dl-author-row">',
                ...
                '</div>',
                '<button class="dl-follow" data-act="follow">关 注</button>',
              '</div>',
            '</div>',
          ...
          '<div class="detail-right">',
            ...
            '<div class="dr-view dr-list-view">',
              ...
              '<div class="dr-list">', renderList(COMMENTS.filter(function (c) { return !c.isReply; })), '</div>',
            '</div>',
            // 详情视图
            '<div class="dr-view dr-detail-view" hidden>',
              ...
              '<div class="dr-detail-body">',
                ...
                '<div class="dr-reply-sep">共 33 条回复</div>',
                renderList(COMMENTS.slice(1)),
              '</div>',
            '</div>',
            // 输入框
  ...
  window.openLivePopup = function (opts) {
    ...
    popup.onclick = function (e) {
      ...
      if (act === 'share-live') showSharePopup(mask, { cover: cover });
      if (act === 'corner-dots') showCornerTip(popup, btn);
      if (act === 'follow') {
  ...
  window.openCreateDreamPopup = function () {
    ...
    mask.innerHTML = [
      '<div class="detail-popup create-dream-popup" role="dialog" aria-modal="true">',
        ...
        '<div class="create-popup-title"><img src="assets/img/icon-history-amber.png" alt="">创建家装方案</div>',
        '<div class="create-popup-body">',
          '<div class="create-upload">',
            '<div class="create-upload-card">',
              '<div class="create-plus">+</div>',
              '<div class="create-upload-text">上传封面</div>',
            '</div>',
            '<img class="create-deco" src="assets/img/deco-owl-flower.png" alt="">',
          '</div>',
  ...
  function bindEvents(mask) {
    ...
    function submitComment() {
      ...
      var prefix = inputMode === 'reply' ? '<span class="cm-reply-prefix">回复 滴栗滴栗：</span>' : '';
      item.innerHTML = [
        '<div class="cm-avatar"></div>',
        '<div class="cm-body">',
    ...
    function showToast(message) {
      var old = popup.querySelector('.detail-toast');
      if (old) old.remove();
  ...
  function toggleLikeImg(btn, kind) {
    ...
    // 同时把数字 +1/-1
    var num = btn.querySelector('.cm-like-num') || btn.querySelector('.db-stat-num');
    if (num) {
  ...
  function showCornerTip(popup, btn) {
    ...
    var tip = document.createElement('div');
    tip.className = 'corner-tip';
    tip.innerHTML = '<span>删除</span><span>编辑</span>';

    var pRect = popup.getBoundingClientRect();
    var bRect = btn.getBoundingClientRect();
    tip.style.top = (bRect.bottom - pRect.top + 10) + 'px';
    tip.style.right = (pRect.right - bRect.right - 64) + 'px';
...
(function enhanceCreateDescScroll() {
  function setup(box) {
    ...
    textarea.addEventListener('input', () => requestAnimationFrame(updateThumb));
