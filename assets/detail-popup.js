...
(function () {
  ...
  /* ---------- 评论 mock 数据 ---------- */
  var COMMENTS = [
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。非常推荐，超级好玩！', time: '2小时前', likes: 999, authorLiked: true, replyCount: 33 },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。非常推荐，超级好玩！非常推荐，超级好玩！', time: '2小时前', likes: 188 },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。', time: '3小时前', likes: 56 },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。非常推荐，超级好玩！', time: '5小时前', likes: 8 },
    { name: '滴栗滴栗', text: '玩家评论，固定展示两行。', time: '昨天', likes: 3 }
  ];
  var REPLIES = Array.from({ length: 33 }, function (_, index) {
    return {
      name: '滴栗滴栗',
      text: '回复 滴栗滴栗：' + (index % 2 ? '玩家回复内容，固定展示两行。' : '谢谢分享，这个家装方案真的很有意思！'),
      time: index < 2 ? '2小时前' : (index < 8 ? '4小时前' : '昨天'),
  ...
  function renderComment(c, idx, hideReplyCount) {
    var prefix = c.isReply
      ? '<span class="cm-reply-prefix">回复 ' + c.name + '：</span>'
      : '';
    var text = c.isReply
      ? '<div class="cm-text">' + prefix + c.text.replace(/^回复\s\S+：/, '') + '</div>'
    ...
    return [
      '<div class="cm-item" data-i="', idx, '">',
        ...
        '<div class="cm-body">',
          ...
          text,
          c.authorLiked ? '<span class="cm-author-liked">作者赞过</span>' : '',
          '<div class="cm-meta">',
            '<span class="cm-time">', c.time, '</span>',
            '<button class="cm-reply-btn">回复</button>',
            ...
            '</span>',
          '</div>',
          (!hideReplyCount && c.replyCount ? '<button class="cm-reply99" data-act="open-replies" data-count="' + c.replyCount + '">' + c.replyCount + ' 条回复</button>' : ''),
        '</div>',
      '</div>'
    ].join('');
  }
  ...
  window.openDetailPopup = function (opts) {
    opts = opts || {};
    var title       = opts.title       || '梦境标题支持最长十二个字';
    var author      = opts.author      || '玩家昵称七个字';
    var cover       = opts.cover       || 'assets/img/card-cover-dream.png';
    var desc        = opts.desc        || '梦境（时空之境）正文简介，固定两行。开启极致画质游玩，感受时空之境的震撼~';
    ...
    mask.innerHTML = [
      '<div class="detail-popup" role="dialog" aria-modal="true">',
        '<div class="detail-corner-btns">',
          '<button class="dc-btn dc-img dc-refresh" title="刷新" data-act="refresh">',
          ...
          '</button>',
          '<button class="dc-btn dc-img dc-dots" title="更多" data-act="corner-dots">',
          ...
          '</button>',
          '<button class="dc-btn dc-close" title="关闭" data-act="close">',
          ...
          '<div class="detail-left">',
            '<div class="dl-card">',
              '<div class="dl-cover" style="background-image:url(\'', cover, '\')">',
                ...
                featured ? '<span class="dl-tag-featured">精选</span>' : '',
              '</div>',
              '<div class="dl-author-row">',
                '<div class="dl-avatar"></div>',
                '<div class="dl-author-info">',
                  '<div class="dl-author-name">', author, '</div>',
                  '<div class="dl-author-time">创建时间：', created, '</div>',
                '</div>',
                ownContent ? '' : '<button class="dl-follow" data-act="follow">关 注</button>',
              '</div>',
            '</div>',
          ...
          '<div class="detail-right">',
            ...
            '<div class="dr-input-bar">',
              ...
              '<div class="dr-input-wrap">',
                '<input class="dr-input" type="text" maxlength="40" placeholder="分享一下你的想法吧~" />',
                '<span class="dr-input-count" hidden>0/40</span>',
              '</div>',
            '</div>',
          '</div>',
        '</div>',
        ...
        '<div class="detail-bottom">',
          ...
          '<div class="db-stats">',
            '<span class="db-stat">',
              '<span class="db-stat-label">已游玩</span>',
              '<span class="db-stat-num-row"><span class="db-stat-num">', played, '</span><span class="db-stat-unit">次</span></span>',
            '</span>',
            '<button class="db-act" data-on="0" data-act="db-star" title="收藏">',
              '<span class="db-stat-label">', imgWithFallback('starBarOff', 14, 14), '收藏</span>',
              '<span class="db-stat-num">', stars, '</span>',
            '</button>',
            '<button class="db-act" data-on="0" data-act="db-like" title="点赞">',
              '<span class="db-stat-label">', imgWithFallback('likeBarOff', 14, 14), '点赞</span>',
              '<span class="db-stat-num">', likes, '</span>',
            '</button>',
          '</div>',
          '<div class="db-actions">',
            '<button class="btn-pill btn-create-blue" data-act="create">创 建</button>',
            '<button class="btn-pill btn-enter-green" data-act="enter">立即进入</button>',
          '</div>',
        '</div>',
      '</div>'
    ].join('');
  ...
  window.openLivePopup = function (opts) {
    ...
    var title = opts.title || 'rp房标题支持最长十二个字';
    var cover = opts.cover || 'assets/img/card-cover-live.png';
    var tag = opts.tag || '玩法标签';
    var featured = opts.featured !== false;
    var ownHistory = opts.ownHistory === true;
    var visibility = opts.visibility || '';
    var liveVisibilityLabels = { public: '公开', friend: '仅粉丝可见', self: '仅自己可见' };
    ...
    if (ownHistory) {
      mask.innerHTML = [
        '<div class="detail-popup live-popup live-popup--history" role="dialog" aria-modal="true">',
          '<div class="detail-corner-btns">',
            '<button class="dc-btn dc-img dc-refresh" title="分享" data-act="share-live"><img src="', IMG.btnRefresh, '" alt=""></button>',
            '<button class="dc-btn dc-close" title="关闭" data-act="close"><img src="assets/img/btn-close.png" alt=""></button>',
          '</div>',
            ...
            '<div class="history-live-left">',
              '<div class="dl-card history-live-card">',
                ...
                '<div class="dl-author-row">',
                  '<div class="dl-avatar"></div>',
                  '<div class="dl-author-info">',
                    '<div class="dl-author-name">玩家昵称七个字</div>',
                    '<div class="dl-author-time">创建时间：2026-02-09</div>',
                  '</div>',
                '</div>',
              '</div>',
              '<p class="history-live-desc">正文简介：固定两行。开启极致画质游玩，感受时空之境的震撼开启极致画质游玩，感受时空之境的震撼~</p>',
              '<div class="history-live-meta">',
                '<span class="history-live-map">这里是地图名称</span>',
                '<span class="history-live-capacity">可参与人数：6</span>',
              '</div>',
            '</div>',
            '<div class="history-live-right">',
              '<div class="history-live-time">',
                '<img src="assets/img/typcn_time.png" alt="">',
                '<div><span>开播时间</span><b>4时32分</b><em>2026-02-09&nbsp;&nbsp;10:03～14:51</em></div>',
              '</div>',
              '<div class="history-live-stats">',
                historyStat('assets/img/history-participants.png', '参与人数', '67'),
                historyStat('assets/img/history-viewers.png', '观众人数', '9999'),
                historyStat('assets/img/history-new-fans.png', '新增粉丝', '143'),
                historyStat('assets/img/history-subscribers.png', '订阅人数', '533'),
                historyStat('assets/img/history-likes.png', '点赞数', '8776'),
                historyStat('assets/img/history-comments.png', '评论数', '244'),
              '</div>',
            '</div>',
          '</div>',
    ...
    mask.innerHTML = [
      '<div class="detail-popup live-popup" role="dialog" aria-modal="true">',
        '<div class="detail-corner-btns">',
          '<button class="dc-btn dc-img dc-refresh" title="分享" data-act="share-live"><img src="', IMG.btnRefresh, '" alt=""></button>',
          '<button class="dc-btn dc-img dc-dots" title="更多" data-act="corner-dots"><img src="', IMG.btnDots, '" alt=""></button>',
          '<button class="dc-btn dc-close" title="关闭" data-act="close"><img src="assets/img/btn-close.png" alt=""></button>',
        '</div>',
            ...
            '<div class="dl-card live-card">',
              ...
              '<div class="dl-author-row">',
                ...
                '<div class="dl-author-info">',
                  ...
                  '<div class="dl-author-time">创建时间：2026-02-09</div>',
                '</div>',
                '<button class="dl-follow" data-act="follow">关注</button>',
              '</div>',
            '</div>',
          '</div>',
          '<div class="live-right">',
            '<div class="live-stats">',
              '<div class="live-stat"><img src="assets/img/typcn_time.png" alt=""><div><span>已开播时间</span><b>4时32分</b></div></div>',
              '<div class="live-stat"><img src="assets/img/Frame 2119903832.png" alt=""><div><span>围观中</span><b>42,376</b></div></div>',
              '<div class="live-stat"><img src="assets/img/Frame 2119903833.png" alt=""><div><span>游玩中</span><b>5/6</b></div></div>',
            '</div>',
            '<div class="live-copy">',
              '<p><b>正文简介：</b>固定两行。开启极致画质游玩，感受时空之境的震撼～梦境</p>',
              '<p><b>地图：</b>这里是地图名称</p>',
              '<p><b>规则：</b>这里是规则这里是规则这里是规则这里是规则这里是规则这里是规则这里是规则这里是规则这里是规则</p>',
            '</div>',
            '<img class="live-deco" src="assets/img/deco-owl-flower.png" alt="">',
          '</div>',
        '</div>',
        '<div class="live-bottom">',
          '<button class="btn-pill btn-create-blue" data-act="switch-live">换一个</button>',
          '<button class="btn-pill btn-enter-green" data-act="enter">立即进入</button>',
        '</div>',
      '</div>'
    ].join('');
    ...
    popup.onclick = function (e) {
      ...
      if (act === 'follow') {
        ...
        btn.textContent = btn.classList.contains('is-on') ? '已关注' : '关注';
      }
    };
  };
    ...
    return [
      '<div class="history-live-stat ', tone || '', '">',
        ...
        '<div><span>', label, '</span><b>', value, '</b>',
          '<small>超过 <strong>99%</strong> 的直播间</small>',
        '</div>',
      '</div>'
    ].join('');
  }
  ...
  window.openCreateDreamPopup = function (opts) {
    ...
    mask.innerHTML = [
      '<div class="detail-popup create-dream-popup" role="dialog" aria-modal="true">',
        ...
        '<div class="create-popup-title"><img src="assets/img/create-dream-title-icon.png" alt="">创建家装方案</div>',
        '<div class="create-popup-body">',
          '<div class="create-upload">',
            '<div class="create-upload-card', draftCover ? ' has-cover' : '', '"', draftCover ? ' style="background-image:url(\'' + draftCover + '\') !important"' : '', '>',
              draftCover
                ? '<button class="create-reselect-btn" type="button">更换封面</button>'
                : '<div class="create-plus">+</div><div class="create-upload-text">上传封面</div>',
            '</div>',
            '<input class="create-cover-input" type="file" accept="image/*" hidden>',
          '</div>',
          '<div class="create-form-panel">',
            '<label class="create-field create-field-title"><span>标题</span><input class="create-title-input" placeholder="请填写梦境标题"><em>0/12</em></label>',
            '<label class="create-field create-field-textarea"><span>介绍</span><div class="create-desc-box"><textarea class="create-desc-input" placeholder="请输入梦境简介"></textarea><em>0/40</em></div></label>',
            '<label class="create-field"><span>玩法类型</span><button type="button" class="create-select">请选择<i></i></button></label>',
            '<div class="create-field"><span>选择地块</span><div class="create-pills"><button>书桌</button><button>雨林</button><button>窗台书桌</button></div></div>',
            '<label class="create-field"><span>最大人数</span><button type="button" class="create-select">请选择<i></i></button></label>',
            '<div class="create-field"><span>权限设置</span><div class="create-radios"><button class="active">所有人</button><button>仅好友</button><button>仅自己</button></div></div>',
          '</div>',
        '</div>',
        '<img class="create-deco" src="assets/img/create-popup-deco.png" alt="">',
        '<div class="create-popup-bottom">',
          '<button class="btn-mini btn-red" data-act="close">取消</button>',
          '<button class="btn-mini btn-green">创建</button>',
        '</div>',
      '</div>'
    ].join('');
    ...
    coverInput?.addEventListener('change', function () {
      ...
      reader.onload = function () {
        ...
        uploadCard.innerHTML = '<button class="create-reselect-btn" type="button">更换封面</button>';
    ...
    mask.querySelectorAll('.create-select').forEach(function (btn) {
      btn.onclick = function () {
        ...
        var old = field.querySelector('.create-dropdown');
        if (old) {
          old.remove();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.innerHTML = (img ? '<img src="assets/img/el_plus-sign.png" alt=""> ' : '') + '创建家装方案';
  });
  ...
  function bindEvents(mask) {
    ...
    popup.addEventListener('click', function (e) {
      ...
      switch (act) {
        ...
        case 'follow':
          btn.classList.toggle('is-on');
          btn.textContent = btn.classList.contains('is-on') ? '已关注' : '关 注';
          break;
        ...
        case 'open-replies':
          ...
          openSep.textContent = '共 ' + openCount + ' 条回复';
    ...
    if (input && input.tagName !== 'TEXTAREA') {
      ...
      textarea.rows = 1;
      textarea.placeholder = '分享一下你的想法吧~';
    ...
    actions.hidden = true;
    actions.innerHTML = '<span class="dr-input-count">0/40</span><button class="dr-send" type="button" data-act="send-comment">发送</button><button class="dr-cancel" type="button" data-act="cancel-input">取消</button>';
    ...
    function updateReplyCount(sourceItem, delta) {
      ...
      button.textContent = countValue + ' 条回复';
      if (source === activeReplySource || detailView.hidden === false) {
        ...
        sep.textContent = '共 ' + countValue + ' 条回复';
      }
      return countValue;
    }
    ...
    function expandInput(mode, sourceItem) {
      ...
      if (mode === 'reply' && !input.value) input.placeholder = '回复 @滴栗滴栗';
      if (mode !== 'reply' && !input.value) input.placeholder = '分享一下你的想法吧~';
      ...
      if (sourceItem) {
        setTimeout(function () {
          ...
          var iRect = bar.getBoundingClientRect();
    ...
    function submitComment() {
      ...
      var prefix = replyingToNestedReply ? '<span class="cm-reply-prefix">回复 滴栗滴栗：</span>' : '';
      item.innerHTML = [
        '<div class="cm-avatar"></div>',
        '<div class="cm-body">',
          '<div class="cm-name">我</div>',
          '<button class="cm-dots" data-act="cm-dots">...</button>',
          '<div class="cm-text">', prefix, escapeHtml(text), '</div>',
          '<div class="cm-meta">',
            '<span class="cm-time">刚刚</span>',
            '<button class="cm-reply-btn">回复</button>',
            '<span class="cm-like-wrap">',
              '<button class="cm-like" data-on="0" data-act="cm-like">',
                imgWithFallback('likeCommentOff', 22, 22),
      ...
      collapseInput();
      showToast(replySep ? '回复已发布' : '评论已发布');
      setTimeout(function () {
        if (replySep) {
    ...
    input.addEventListener('blur', function () {
      wrap.classList.remove('is-focus');
      if (!input.value) count.hidden = true;
    });
    input.addEventListener('input', function () {
      // maxlength 兜底
      // Allow over-40 input; the counter turns red instead of truncating.
      count.textContent = input.value.length + '/40';
    });
    input.addEventListener('input', function () {
      count.textContent = input.value.length + '/40';
      count.classList.toggle('is-over', input.value.length > 40);
      setInputHeight();
    });
  ...
  document.addEventListener('click', function (e) {
    ...
    var title = (card.querySelector('.card-title, .card-title-bar .title, .title') || {}).textContent || '梦境标题支持最长十二个字';
    var tag   = (card.querySelector('.card-tag-left') || {}).textContent || '玩法标签';
  ...
  function showSharePopup(parentMask, opts) {
    ...
    var channels = [
      { label: '保存到本地', icon: 'assets/img/icon.png' },
      { label: '口令码', icon: 'assets/img/icon-1.png' },
      { label: '微信', icon: 'assets/img/Group 2119903741.png' },
      { label: '朋友圈', icon: 'assets/img/Group 2119903741-1.png' },
      { label: 'QQ', icon: 'assets/img/Group 2119903741-2.png' },
      { label: 'QQ空间', icon: 'assets/img/Group 2119903742.png' },
      { label: '抖音', icon: 'assets/img/Group 2119903743.png' },
      { label: '快手', icon: 'assets/img/Group 2119903744.png' },
      { label: 'B站', icon: 'assets/img/Group 2119903745.png' }
    ];
    ...
    mask.innerHTML = [
      '<div class="share-panel" role="dialog" aria-modal="true">',
        '<button class="share-close" type="button" aria-label="关闭"><img src="assets/img/btn_close_2k 3.png" alt=""></button>',
  ...
  function showCornerTip(popup, btn, showOwnerActions) {
    ...
    tip.innerHTML = showOwnerActions
      ? '<button type="button">删除</button><button type="button">编辑</button>'
  ...
  function showCommentTip(popup, btn) {
    ...
    var tip = document.createElement('div');
    tip.className = 'comment-tip';
    tip.innerHTML = '<span>删除</span><span>举报评论</span>';
  ...
  document.addEventListener('click', function (e) {
    ...
    window.openLivePopup({
      title: (titleEl && titleEl.textContent.trim()) || '直播标题最长支持十二个字',
      cover: coverImg || 'assets/img/card-cover-live.png',
      tag: (tagEl && tagEl.textContent.trim()) || '玩法标签',
